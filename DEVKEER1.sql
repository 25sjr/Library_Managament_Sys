...
select PUBLICATION,COUNT(PUBLICATION)as NO_OF_BOOKS from LMS_BOOK_DETAILS WHERE PUBLICATION='AAA';
...
GROUP BY BOOK_TITLE;
  ...
  /*p-6*/
 SELECT  MEMBER_NAME,MEMBER_ID, DATE_RETURN AS DUE_DATE, DATE_RETURNED AS RETURN_DATE
  FROM LMS_MEMBERS NATURAL JOIN lms_book_issue WHERE (DATE_RETURN < DATE_RETURNED);

  /*p-7*/
  select MEMBER_ID,MEMBER_NAME,DATE_REGISTER
  case when BOOK_ISSUE is NULL then 'NO' when CONTACT is not null then 'YES' end as PHONE_NUM_AVAILABLE from LMS_SUPPLIER_DETAILS ;
